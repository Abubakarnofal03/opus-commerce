...
Rules: 1.Submit correct and effective andriod package which can be downloaded; 2.andriod package and the andriod url should direct to the same app.","Optional || The name of each Android app || Submit correct android app name, matched the android package and direct to the same app.",Optional | A custom label for each product,Optional | A custom label for each product,Optional | A custom label for each product,Optional | A custom label for each product,Optional | A custom label for each product`;
    ...
    const productRows = products?.map((product: any, index: number) => {
      ...
      // Build row - first product row has "example" prefix, rest have empty prefix
      const rowPrefix = index === 0 ? 'example' : '';
      
      // All 45 columns (including prefix column)
      const columns = [
        rowPrefix,
        escapeCsv(product.id), // sku_id
        escapeCsv(cleanTitle), // title
        escapeCsv(cleanDescription), // description
        availability, // availability (lowercase)
        'new', // condition (lowercase as per TikTok example)
    ...
    // Combine all rows
    const csvContent = [row1, row2, row3, ...productRows].join('\n');
